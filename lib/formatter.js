...
function formatDiv(elem, fn, options) {
  var startingNewLine = options.needsStartingNewLine ? '\n' : '';
  var endingNewLine = childDivExists(elem) ? '': '\n';
  return startingNewLine + fn(elem.children, options) + endingNewLine;
}

function formatSpan(elem, fn, options) {
  var tab = options.needsTab ? '\t' : '';
  return tab + fn(elem.children, options);
}

function childDivExists(elem) {
  var queue = [];
