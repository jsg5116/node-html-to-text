        ...
        switch(elem.name.toLowerCase()) {
          ...
          case 'div':
            ...
            if (result && !result.endsWith('\n')) {
              newOptions.needsStartingNewLine = true;
            }
            else {
              newOptions.needsStartingNewLine = false;
            }

            result += format.div(elem, walk, newOptions);
            break;
          case 'span':
            newOptions = Object.assign({}, options);
            if (result && !result.endsWith('\n')) {
              newOptions.needsTab = true;
            }
            else {
              newOptions.needsTab = false;
            }
            result += format.span(elem, walk, newOptions);
            break;
          default:
            result = walk(elem.children || [], options, result);
        }
